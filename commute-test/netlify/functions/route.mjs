...
async function geocode(query,key){const headers={Authorization:'KakaoAK '+key};let response=await fetch('https://dapi.kakao.com/v2/local/search/address.json?query='+encodeURIComponent(query),{headers});let data=await response.json().catch(()=>({}));if(response.ok&&!data?.documents?.length){response=await fetch('https://dapi.kakao.com/v2/local/search/keyword.json?query='+encodeURIComponent(query),{headers});data=await response.json().catch(()=>({}))}if(!response.ok)throw new Error('카카오 주소검색 오류 ('+response.status+'): '+(data.msg||data.message||data.error||'키 설정을 확인하세요'));if(!data?.documents?.length)throw new Error('주소를 찾지 못했습니다: '+query);return{x:data.documents[0].x,y:data.documents[0].y}}
